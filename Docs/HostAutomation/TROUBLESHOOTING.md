...
| Installer reports access denied | Preview again with `-DryRun`, then run PowerShell 7 elevated as the same `02031` identity. Program Files staging and ACL protection require administrative authority; do not change task identity. |
...
| Bundle manifest, hash, or ACL fails | Do not edit, re-ACL, or run the staged copy. Review the source files and config, rerun installer `-DryRun`, then install a new content-addressed bundle from an elevated session as `02031`. |
...
| Executable identity or absolute-path check fails | Do not substitute a name found through `PATH` or update the identity file by hand. Verify the configured Git, Git LFS, GitHub CLI, Codex, Unity, and stable PowerShell files, then review and install a new protected bundle so all six canonical paths, lengths, and SHA-256 hashes are rebound together. |
...
| Git configuration isolation fails | Do not restore user/system Git config, hooks, fsmonitor, external helpers, or PATH-based LFS. Verify the exact bound Git, Git LFS, and GitHub CLI paths plus configured author identity, then start a fresh run. |
...
| Codex command audit rejects a command | Treat this as containment evidence. Codex command events may use only the recognized read-only inspection surface; Host-owned Git/GitHub/Unity operations, interpreters, wrappers, network clients, absolute/traversal paths, redirection, and opaque shell syntax are rejected. Do not weaken the audit or OS sandbox to make the turn pass. |
| Codex capability probe fails | Upgrade or configure the installed CLI only after verifying it supports the required exec, JSON, ephemeral, sandbox, approval, and Windows unelevated contract. Never use a dangerous bypass flag. |
| Codex authentication fails but interactive login worked previously | The Host intentionally removes environment API keys and other credential-shaped variables before launching Codex. Authenticate the installed CLI through its current-user credential store. Do not restore `OPENAI_API_KEY`, `CODEX_API_KEY`, GitHub tokens, askpass, SSH-agent, cloud-credential, or authenticated-proxy variables to the child environment. |
...
| Unity timeout | Inspect the stage log and recorded run-owned PID. The Host should terminate only that process, mark validation failed, and retain evidence. |
...
For New Work, failure leaves the Issue `In Progress` once that start
transition has legitimately occurred. For ReviewFix or DeliveryResume, failure
also retains `In Progress`. The Host publishes only sanitized evidence that is
safe for the current pin.
