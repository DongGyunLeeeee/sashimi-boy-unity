...
| Generator nondeterminism | Inspect `Determinism.Comparisons` and both original delta files. Only Issue #20's three exact Stage01 salmon preview PNGs may use the Owner-approved RGB 1/255 and changed-pixel 0.1% limit; alpha, dimensions, metadata and all other files remain exact. Both Reviewer committed comparisons are required. Out-of-policy or post-evidence changes still fail; fix the authoritative generator/input instead of weakening the gate or committing an arbitrary result. |
